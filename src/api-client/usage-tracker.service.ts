 ...
 * Redis key 格式: `usage:daily:{clientId}:{YYYYMMDD}`
...
export class UsageTrackerService {
  ...
  async recordRequest(clientId: string): Promise<void> {
    ...
    try {
      ...
      this.logger.warn(
        `记录请求用量失败: clientId=${clientId}, error=${(error as Error).message}`,
      );
    }
  }
  ...
  async recordCompletion(
    ...
    try {
      ...
      this.logger.warn(
        `记录完成用量失败: clientId=${clientId}, success=${success}, error=${(error as Error).message}`,
      );
    }
  }
  ...
  async getUsage(
    ...
    for (const date of dates) {
      const key = `usage:daily:${clientId}:${date}`;
      try {
        ...
        this.logger.warn(
          `查询用量失败: clientId=${clientId}, date=${date}, error=${(error as Error).message}`,
        );
        results.push({
          clientId,
          date,
  ...
  async getUsageSummary(): Promise<ClientUsageSummary[]> {
    ...
    try {
      ...
      for (const key of keys) {
        // key 格式: usage:daily:{clientId}:{date}
  ...
  private buildKey(clientId: string): string {
    return `usage:daily:${clientId}:${this.getTodayStr()}`;
  }
